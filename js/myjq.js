$(document).ready(function(){
	...
	function showPhoto(){
		...
		for (var i = 0; i < photoArr.length; i++) {
			photoArr[i].style.background = 'url("images/person/photo-'+ i +'.png")';
	...
	function showArrow(arrowIndex){
		if(arrowIndex == 0){
			goLeft.css('background','url(images/left-after.png)');
		}else{
			goLeft.css('background','url(images/left.png)');
		}
		if(arrowIndex!=8){
			goRight.css('background','url(images/right.png)');
		}
		else{
			goRight.css('background','url(images/right-after.png)');
		}
	}
	//实现下拉footer事件
	//获取下拉箭头图标
